...
/**
 * Voyage-code-3 Embeddingクライアント。
 *
 * <p>コード用Embeddingモデル。Java/XML最適化、CoIR 77.33。
 * OpenAI互換のHTTP APIを呼び出し、1024次元のベクトルを生成する。</p>
 *
 * <h3>有効化条件</h3>
 * <p>{@code nablarch.mcp.embedding.provider=api} の場合に有効化される。
 * ローカルモデル（CodeSage）への移行により、通常はlocalを推奨する。</p>
 */
@Component
@Qualifier("code")
...
public class VoyageEmbeddingClient implements EmbeddingClient {
    ...
    private static final Logger logger = LoggerFactory.getLogger(VoyageEmbeddingClient.class);
     ...
     */
    public VoyageEmbeddingClient(EmbeddingProperties properties) {
        this.config = properties.getVoyage();
        ...
        while (true) {
            try {
                ...
                if (response == null || response.getData() == null) {
                    throw new EmbeddingException("Voyage API returned null response");
                }

                logger.debug("Voyage API: {} テキストのEmbedding生成完了", texts.size());
                ...
                if (retries > config.getMaxRetries()) {
                    throw new EmbeddingException(
                            "Voyage API call failed after " + config.getMaxRetries() + " retries", e);
                }
                logger.warn("Voyage API呼び出し失敗（リトライ {}/{}）: {}",
