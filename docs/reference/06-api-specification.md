...
全capabilitiesは静的（`listChanged: false`）です。
...
全リソースはMCP標準の `resources/read` メソッドでアクセスします。レスポンス形式はリソースにより異なります（`text/markdown`, `application/json`, `text/plain`）。
...
1. **バージョン固有の知識なし**: migration-guideプロンプトはモジュール一覧を提供するが、バージョン間の差分情報は含まない
2. **Nablarch 6系のみ**: 知識ベースはNablarch 6系を対象。5系以前の固有情報は限定的
3. **日本語出力**: ツールのレスポンスは日本語。多言語サポートなし

### 実装済み機能

以下は既に実装済みです:

- **RAGエンジン統合**: pgvectorによるベクトル検索 + BM25（日本語FTS対応）のハイブリッド検索
- **Streamable HTTPトランスポート**: HTTPモードによるリモート接続サポート（`--spring.profiles.active=http`）
- **ONNX Embedding**: ローカルONNXモデル（BGE-M3 / CodeSage）によるEmbedding生成
- **公式ドキュメント連携**: Nablarch公式ドキュメント（467ページ・1485チャンク）の自動取込
- **一次ソースURL付与**: 検索結果にNablarch公式ドキュメントURLを付与
