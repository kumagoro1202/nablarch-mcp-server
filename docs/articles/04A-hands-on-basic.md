...
4. [Tool 2: design_handler_queue --- ハンドラキューを自動設計](#4-tool-2-design_handler_queue----ハンドラキューを自動設計)
...
- **design_handler_queue**: アプリケーションタイプと要件からハンドラキューXMLを自動生成
...
| カテゴリ | 本記事で体験 | 応用編で解説 |
|---------|------------|------------|
| Tools（10種） | semantic_search, design_handler_queue, search_api | validate_handler_queue, generate_code, generate_test, optimize_handler_queue, recommend_pattern, analyze_migration |
...
## 4. Tool 2: design_handler_queue --- ハンドラキューを自動設計

次に試すのは **design_handler_queue** ツールです。アプリケーションタイプと要件を指定するだけで、Nablarchのベストプラクティスに準拠したハンドラキューXML設定を自動生成します。
...
MCP InspectorのToolsタブから `design_handler_queue` を選択し、以下のパターンで試してみましょう。
...
| **design_handler_queue** | ハンドラキュー自動設計 | 「要件を伝えるだけでXML設定が出てくる」 |
