...
2. [テスト全体像 — 1,029件のテストの内訳](#2-テスト全体像--1029件のテストの内訳)
...
本記事では、Nablarch MCP Serverの**1,029件のテスト**がどのように品質を守っているかを解説します。
...
```mermaid
graph TD
    A[E2Eテスト<br/>約50件<br/>Claude Desktopとの統合動作検証] --> B[統合テスト<br/>約250件<br/>RAGパイプライン・検索品質評価]
    B --> C[単体テスト<br/>約719件<br/>個別コンポーネントの動作検証]
...
| レイヤー | テスト数 | 実行時間 | 目的 |
|---------|---------|---------|------|
| **単体テスト** | 約719件 | 高速（秒単位） | 個別クラス・メソッドの正常系・異常系 |
| **統合テスト** | 約250件 | 中速（10秒単位） | RAGパイプライン・検索品質評価 |
| **E2Eテスト** | 約50件 | 低速（分単位） | MCPプロトコル・Tool/Resource統合 |
| **合計** | **1,029件** | **~2分** | **1,015件成功、14件スキップ** |
...
- **Phase 4（品質基盤）**: +219件（ONNX Embedding、Actuator、HealthIndicator、構造化ログ、相関ID、日本語FTS）

### 2.3 テスト成功率

最終テスト実行結果（2026-02-12）：

```
Tests run: 1029, Failures: 0, Errors: 0, Skipped: 14
Success rate: 98.63%
```

**スキップ14件の内訳**：
- OnnxEmbeddingClientTest: 8件（ONNXモデルファイル未配置のため条件付きスキップ）
- SearchQualityEvaluationTest: 1件（評価データセット50件の準備未完了）
- HealthIndicatorTest: 3件（PostgreSQL接続が必要なため条件付きスキップ）
- CorrelationIdFilterTest: 2件（HTTPモードのみ対象のため条件付きスキップ）
...
✅ **1,029件のテストが守る4層構造**：
- 単体テスト（~719件）：個別コンポーネントの正常系・異常系
- 統合テスト（~250件）：RAGパイプライン・検索品質評価
