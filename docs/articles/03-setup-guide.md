...
> **難易度**: 初級
> **所要時間**: 30分
> **前提記事**: [03A. Nablarch入門](./03A-nablarch-introduction.md)
...
この記事では、環境構築からサーバー起動、AIツールへの接続設定までを一通り解説します。全10ツール・8種のResource・全6プロンプトが単一構成で起動します。
...
| `generate_code` | Nablarchコード生成 |
| `generate_test` | テストコード生成 |
| `analyze_migration` | バージョン移行分析 |
...
| **YAML知識ベース** | 9個 | `search_api`, `validate_handler_queue`, `design_handler_queue`, `optimize_handler_queue`, `generate_code`, `generate_test`, `analyze_migration`, `recommend_pattern`, `troubleshoot` | `knowledge/` 配下のYAMLファイル |

### リソースとプロンプト

| カテゴリ | 個数 | DB依存 |
|---------|------|--------|
| **Resources** | 8種 | なし（全てYAML知識ファイルベース） |
...
| **サーバー起動** | 全10ツール・8種のResource・6プロンプトが単一構成で起動 |
...
| 記事 | 内容 |
|------|------|
| **[04A. 使ってみよう 基本編](./04A-hands-on-basic.md)** | 最初の10分で感動体験 |
| **[04B. 使ってみよう 応用編](./04B-hands-on-advanced.md)** | 10のToolsを使いこなす |
| **[05. アーキテクチャ概要](./05-architecture-overview.md)** | Spring Boot + MCP + RAGの三位一体 |
| **[10. Tool設計と実装パターン](./10-tool-design-patterns.md)** | 10 Toolsの設計思想を読み解く |
...
- [前の記事: 03A. Nablarch入門](./03A-nablarch-introduction.md)
- [次の記事: 04A. 使ってみよう 基本編](./04A-hands-on-basic.md)
- [シリーズ目次に戻る](./INDEX.md)
